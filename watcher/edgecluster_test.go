		...
		It("should create corresponding porch packages", func(ctx SpecContext) {
			...
			for i := 1; i <= 100; i++ {
				go func(i int) {
					...
					expectedYaml := fmt.Sprintf(yamlFormat, testEnv.GetNamespace(), clusterName, timeStamp.Format(time.RFC3339), podIP, port, testEnv.GetNamespace(), testEnv.GetNamespace(), testEnv.GetNamespace())
